...
export class Museum3D {
  ...
  placeArtworksInWorld(artworks) {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      // ALL display materials are UNLIT (MeshBasicMaterial): Marble worlds carry no scene
      // lighting, so any PBR material renders near-black there — that is exactly how the
      // "white glass" pane came out black (user report). Unlit white reads as translucent
      // white glass in every world, lit or not.
      const panel = new THREE.Mesh(new THREE.BoxGeometry(1.8, 2.5, 0.06), new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.14, depthWrite: false, toneMapped: false, side: THREE.DoubleSide }));
      panel.position.set(0, 1.25, 0);
      panel.userData.artwork = artwork;
      group.add(panel);

      // Opaque matte backing behind the canvas (passe-partout). It WRITES DEPTH, so gaussians
      // behind the artwork are occluded instead of smearing over it during camera motion and
      // lingering as dark patches after stopping (user report) — the original box gallery was
      // always crisp for exactly this reason: art sat on opaque walls.
      // Z-STACK (strictly ordered, front faces must never pass the canvas): glass panel front
      // ends at 0.03 -> opaque mat plane 0.05 -> frame box 0.05..0.09 -> canvas 0.10. The first
      // cut had the frame's front face at 0.125, IN FRONT of the canvas at 0.12 - the whole
      // unit rendered as a solid white slab with the artwork invisible (user report).
      const matMat = new THREE.MeshBasicMaterial({ color: 0xf7f5ef, toneMapped: false });
      const backing = new THREE.Mesh(new THREE.PlaneGeometry(1, 1), matMat);
      backing.position.set(0, 1.5, 0.05);
      backing.scale.set(1.48, 1.16, 1);
      backing.userData.artwork = artwork;
      group.add(backing);

      const frame = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 0.04), new THREE.MeshBasicMaterial({ color: 0xf0f2f4, toneMapped: false }));
      frame.scale.set(1.3, 0.98, 1);
      frame.position.set(0, 1.5, 0.07);
      ...
      painting.position.set(0, 1.5, 0.1);
