...
export class Museum3D {
  constructor({ container, artworks, companions = [], onArtworkFocus, onCompanionSelect = null, onReady, onWorldReady = null, onTourUpdate = null, world = null }) {
    // Guided tour: the HUD gets the next stop's name, bearing and distance from emitTourUpdate.
    this.onTourUpdate = onTourUpdate;
    this.tourIndex = 0;
  ...
  placeArtworksInWorld(artworks) {
    ...
    // TOUR LAYOUT (user: lead me from one painting to the next). Artworks hang in TOUR ORDER
    // along the direction the visitor spawns facing: the first ~7m ahead, then every ~6.5m,
    // alternating left and right of the walk line, each turned back toward that line. Walking
    // straight forward therefore meets them one at a time, in the scene's own order.
    const yaw0 = this.worldProfile.yaw || 0;
    const fwd = new THREE.Vector3(Math.sin(yaw0), 0, -Math.cos(yaw0));
    const right = new THREE.Vector3(Math.cos(yaw0), 0, Math.sin(yaw0));
    const spawn = this.worldProfile.spawn;
    const loader = new THREE.TextureLoader();
    loader.setCrossOrigin("anonymous");
    const solidWall = this.worldMesh || this.worldCollider;
    const LATERAL = 3.2; // stand-off from the walk line when there is no wall to hang on
    const n = Math.min(list.length, 8);
    for (let i = 0; i < n; i += 1) {
      const artwork = list[i];
      const sideSign = i % 2 === 0 ? -1 : 1;             // alternate the two sides of the walk
      // A stop must stand INSIDE the captured world (user: in the water garden the frames hung
      // out beyond the model). Walk out only as far as real ground carries us: try the planned
      // reach, then progressively shorter ones, and take the first that has collider ground
      // both under the stop and under the frame's stand-off position.
      const planned = 7 + i * 6.5;
      let reach = planned * 0.35;
      for (const factor of [1, 0.8, 0.62, 0.48, 0.35]) {
        const r = planned * factor;
        const tx = spawn.x + fwd.x * r;
        const tz = spawn.z + fwd.z * r;
        if (this.hasGroundAt(tx, tz) && this.hasGroundAt(tx + right.x * sideSign * LATERAL, tz + right.z * sideSign * LATERAL)) { reach = r; break; }
      }
      const cx0 = spawn.x + fwd.x * reach;
      const cz0 = spawn.z + fwd.z * reach;
      const sideDir = new THREE.Vector3(right.x * sideSign, 0, right.z * sideSign);
      ...
      // returns wildly different distances (rejected). Try the stop, then shifted +-2.5m
      // along the walk; no solid wall anywhere -> freestanding at LATERAL.
      const wallDistAt = (ax, az) => {
        if (!solidWall) return null;
        const gy = this.groundAt(ax, az);
        const probes = [[0, 1.5], [0, 0.9], [0, 2.0], [-0.6, 1.5], [0.6, 1.5]]; // (alongOffset, height)
        const dists = [];
        for (const [da, dy] of probes) {
          const ox = ax + fwd.x * da;
          const oz = az + fwd.z * da;
      ...
      let wx = cx0, wz = cz0, wallDist = null;
      for (const shift of [0, 2.5, -2.5]) {
        const ax = cx0 + fwd.x * shift, az = cz0 + fwd.z * shift;
        const d = wallDistAt(ax, az);
        if (d !== null && d > 1.2) { wallDist = d; wx = ax; wz = az; break; }
      }
      // TIER 2 (user: the frames should still back onto something). A garden has no flat dense
      // wall, so the strict five-probe test above always rejects it and the frame ends up
      // floating in the open. Fall back to a single centre ray: whatever solid it meets —
      // hedge, bank, colonnade — the frame backs onto it, standing 0.55m proud so foliage
      // cannot swallow the canvas.
      if (wallDist === null && solidWall) {
        this.raycaster.set(new THREE.Vector3(wx, this.groundAt(wx, wz) + 1.5, wz), sideDir);
        this.raycaster.far = 14;
        const near = this.raycaster.intersectObject(solidWall, true)[0];
        this.raycaster.far = Infinity;
        if (near && near.distance > 1.4) {
          const backed = near.distance - 0.55;
          if (this.hasGroundAt(wx + sideDir.x * backed, wz + sideDir.z * backed)) wallDist = near.distance;
        }
      }
      // CLEAR THE WHOLE FRAME, not just its centre. A curved or lumpy wall (the dot room, a
      // garden bank) buries the edges of a ~1.7m-wide frame that is only tucked in at the
      // middle — user: the scene 8 frames were stuck in the wall. Measure the wall at both
      // edges as well and back off from the TIGHTEST of the three.
      if (wallDist !== null && solidWall) {
        let nearest = wallDist;
        for (const edge of [-0.85, 0.85]) {
          const ex = wx + fwd.x * edge, ez = wz + fwd.z * edge;
          this.raycaster.set(new THREE.Vector3(ex, this.groundAt(ex, ez) + 1.5, ez), sideDir);
          this.raycaster.far = wallDist + 6;
          const h = this.raycaster.intersectObject(solidWall, true)[0];
          this.raycaster.far = Infinity;
          if (h) nearest = Math.min(nearest, h.distance);
        }
        wallDist = Math.max(1.2, nearest - 0.5); // 0.5m of air between frame and wall
      }
      let off = wallDist !== null ? wallDist : LATERAL;
      if (wallDist === null) {
        // Nothing to back onto: pull the stand-off in (and, failing that, swap sides) until the
        // frame at least stands over real ground rather than out past the edge of the capture.
        for (const candidate of [LATERAL, 2.2, 1.4, -LATERAL, -2.2]) {
          if (this.hasGroundAt(wx + sideDir.x * candidate, wz + sideDir.z * candidate)) { off = candidate; break; }
        }
      }
      let px = wx + sideDir.x * off;
      let pz = wz + sideDir.z * off;
      // Freestanding frames stay inside the walk bounds (inset 0.4m); wall-snapped ones sit AT
      // the wall, which legitimately lies beyond the inset walk area.
      if (wallDist === null) { px = clamp(px, b.minX + 0.4, b.maxX - 0.4); pz = clamp(pz, b.minZ + 0.4, b.maxZ - 0.4); }
      ...
      group.rotation.y = Math.atan2(wx - px, wz - pz);                      // turn back toward the walk line
  ...
  // True when the hidden collider really carries ground at (x,z) — as opposed to groundAt()
  // returning its flat-profile fallback because the ray met nothing. Artwork placement uses it
  // to stay inside the captured world instead of hanging frames over the void.
  hasGroundAt(x, z) {
    if (!this.worldCollider) return true; // no collider to consult: do not block placement
    const gy0 = this.worldProfile?.groundY || 0;
    const ws = this.worldScale || 1;
    this.raycaster.set(new THREE.Vector3(x, gy0 + 300, z), DOWN);
    this.raycaster.far = 600;
    const hits = this.raycaster.intersectObject(this.worldCollider, true);
    this.raycaster.far = Infinity;
    return hits.some(h => Math.abs(h.point.y - gy0) <= 6 * ws);
  }

  // ---- Guided tour ----------------------------------------------------------------------
  // The visitor is walked from artwork to artwork: the HUD names the next stop with a bearing
  // arrow and a live distance, and once they are standing at it the companions leave their
  // walking slots and gather beside that frame. advanceTour() is called by the app when the
  // stop has been discussed.

  tourTarget() { return this.paintings[this.tourIndex] || null; }

  advanceTour() {
    if (!this.paintings.length) return;
    this.tourIndex = Math.min(this.tourIndex + 1, this.paintings.length - 1);
    this.tourArrived = false;
    this.emitTourUpdate();
  }

  emitTourUpdate() {
    const stop = this.tourTarget();
    if (!stop || !this.onTourUpdate) return;
    const cam = this.camera.position;
    const dx = stop.group.position.x - cam.x;
    const dz = stop.group.position.z - cam.z;
    const distance = Math.hypot(dx, dz);
    // Bearing RELATIVE to where the visitor is looking: 0 = dead ahead, positive = to the right.
    const len = distance || 1;
    const nx = dx / len, nz = dz / len;
    const fx = Math.sin(this.yaw), fz = -Math.cos(this.yaw);
    const rx = Math.cos(this.yaw), rz = Math.sin(this.yaw);
    const angleDeg = Math.atan2(nx * rx + nz * rz, nx * fx + nz * fz) * (180 / Math.PI);
    const arrived = distance < 3.4;
    this.tourArrived = arrived;
    // Only wake the DOM when something a viewer would notice changed.
    const signature = `${this.tourIndex}|${arrived}|${distance.toFixed(1)}|${Math.round(angleDeg / 4)}`;
    if (signature === this.tourSignature) return;
    this.tourSignature = signature;
    this.onTourUpdate({
      index: this.tourIndex,
      total: this.paintings.length,
      title: stop.artwork?.title || "",
      artist: stop.artwork?.artist || "",
      distance,
      angleDeg,
      arrived,
    });
  ...
  loadCompanionModels(companions) {
    this.companionParty = [];
    // Take the first three CHOSEN companions — do not filter on `model`. Two roster members
    // (hilma-af-klint, berthe-morisot) ship model:null and were silently dropped here, so a
    // visitor who invited them walked with a missing person (user report). They now get a
    // portrait standee instead, and so does any companion whose GLB fails to load.
    const chosen = companions.slice(0, 3);
    if (!chosen.length) return;

    const loader = new GLTFLoader();
    chosen.forEach((companion, index) => {
      // Shared placement for real GLBs and standees alike.
      const place = (avatar, footOffset) => {
        if (this.disposed) return;
      ...
      if (!companion.model) { place(this.makeCompanionStandee(companion), 0); return; }
      ...
      loader.load(
        ...
        gltf => {
          ...
          place(avatar, footOffset);
        },
        undefined,
        error => {
          console.warn(`Companion model unavailable for ${companion.fullName}`, error);
          place(this.makeCompanionStandee(companion), 0); // a chosen master must never vanish
        }
      );
    });
  }

  // Museum-style portrait standee for companions without a reviewed 3D model: their real
  // public-domain portrait at human height on an easel-like board. Unlit so it reads in any
  // world; DoubleSide so the face-the-visitor rotation always shows the picture.
  makeCompanionStandee(companion) {
    const group = new THREE.Group();
    const mat = new THREE.MeshBasicMaterial({ color: 0xdad5cc, side: THREE.DoubleSide, toneMapped: false });
    const board = new THREE.Mesh(new THREE.PlaneGeometry(1, 1), mat);
    board.scale.set(1.05, 1.55, 1);
    board.position.y = 0.875; // bottom edge sits on the ground the caller foots us on
    group.add(board);
    const trim = new THREE.Mesh(new THREE.BoxGeometry(1.13, 1.63, 0.03), new THREE.MeshBasicMaterial({ color: 0x5e4028, toneMapped: false }));
    trim.position.set(0, 0.875, -0.02);
    group.add(trim);
    if (companion.portrait) {
      const texLoader = new THREE.TextureLoader();
      texLoader.load(companion.portrait, (texture) => {
        texture.colorSpace = THREE.SRGBColorSpace;
        mat.map = texture;
  ...
  animate = () => {
    ...
    this.tick = (this.tick || 0) + 1; // this.frame is the rAF handle, not a counter
    if (this.worldReady && (this.tick & 7) === 0) this.emitTourUpdate(); // ~8fps is plenty for a HUD
  ...
  updateCompanions() {
    ...
    // Standing AT a tour stop, the masters stop shadowing the camera and gather in front of
    // that frame, flanking it so the canvas itself stays clear (user: send them to the
    // painting). They keep facing the visitor, so the faces stay readable.
    const stop = this.tourArrived ? this.tourTarget() : null;
    let stopBase = null, stopPerp = null;
    if (stop) {
      const sp = stop.group.position;
      const vx = sp.x - cam.x, vz = sp.z - cam.z;
      const vl = Math.hypot(vx, vz) || 1;
      const ux = vx / vl, uz = vz / vl;
      stopBase = { x: sp.x - ux * 1.9, z: sp.z - uz * 1.9 }; // stand ~1.9m out from the wall
      stopPerp = { x: uz, z: -ux };
    }
    for (let ci = 0; ci < this.companionParty.length; ci += 1) {
      const c = this.companionParty[ci];
      const a = c.avatar;
      let tx = cam.x - fwdX * c.back + rgtX * c.side;
      let tz = cam.z - fwdZ * c.back + rgtZ * c.side;
      if (stopBase) {
        const spread = [-1.55, 1.55, -2.9][ci % 3]; // two flanking the frame, the third wider
        tx = stopBase.x + stopPerp.x * spread;
        tz = stopBase.z + stopPerp.z * spread;
      }
