...
export class Museum3D {
  ...
  buildArchitecture() {
    ...
    const leaf = new THREE.MeshStandardMaterial({ color: 0x55745f, roughness: 0.9 });
    const rose = new THREE.MeshStandardMaterial({ color: 0xc88998, roughness: 0.86 });
    for (const z of [2, -14, -30, -46]) {
      for (const side of [-1, 1]) {
        const planter = new THREE.Mesh(new THREE.BoxGeometry(1.8, 0.55, 1.25), stone);
        planter.position.set(side * 5.45, 0.28, z);
        this.scene.add(planter);
        for (let i = 0; i < 7; i += 1) {
          const foliage = new THREE.Mesh(new THREE.SphereGeometry(0.34 + (i % 3) * 0.06, 12, 10), i % 3 === 0 ? rose : leaf);
          foliage.position.set(side * 5.45 + (i % 3 - 1) * 0.38, 0.72 + (i % 2) * 0.2, z + (Math.floor(i / 3) - 0.5) * 0.35);
  ...
  async buildWorldEnvironment() {
    ...
    const SPLAT_SCALE = 7.5;
    const SPLAT_POSITION = [0, 1.5, -30];
    const SPLAT_ROTATION_Y = Math.PI;
    try {
      ...
      // Marble worlds use the OpenCV frame (+y down, +z forward); convert to
      // Three.js by negating Y and Z (X unchanged) — folded into the env scale,
      // otherwise the world renders upside-down/mirrored. (Spark/World Labs docs)
      this.worldSplat.scale.set(SPLAT_SCALE, -SPLAT_SCALE, -SPLAT_SCALE);
      this.worldSplat.position.set(...SPLAT_POSITION);
      this.worldSplat.rotation.y = SPLAT_ROTATION_Y;
      this.scene.add(this.worldSplat);
      // Let the generated world show beyond the colonnade: darker sky, thinner fog.
      this.scene.background = new THREE.Color(0x0d0b14);
      if (this.scene.fog) this.scene.fog.density = 0.0055;
    } catch (error) {
  ...
  bindEvents() {
    ...
    this.handlers = {
      ...
      wheel: event => {
        ...
        this.camera.position.z = clamp(this.camera.position.z + event.deltaY * 0.012, -49, 13);
      },
  ...
  animate = () => {
    ...
    this.camera.position.z = clamp(this.camera.position.z, -49, 13);
