...
export class Museum3D {
  constructor({ container, artworks, companions = [], onArtworkFocus, onReady, worldSplatUrl = null, worldMeshUrl = null }) {
  ...
  // Preferred crisp path: a Marble "High-quality mesh" (textured .glb) as the walkable
  // environment. Meshes occlude the character meshes cleanly (unlike splats) and stay
  // sharp up close. Auto-fits any mesh via its bounding box, so no per-world tuning.
  async buildWorldMesh() {
    try {
      const gltf = await new GLTFLoader().loadAsync(this.worldMeshUrl);
      if (this.disposed) return false;
      const mesh = gltf.scene;
      mesh.rotation.x = Math.PI; // Marble OpenCV(+y down) -> three.js
      mesh.updateMatrixWorld(true);
      // Scale so the room's larger horizontal span comfortably surrounds the walk area.
      const size = new THREE.Box3().setFromObject(mesh).getSize(new THREE.Vector3());
      const TARGET_SPAN = 34;
      mesh.scale.setScalar(TARGET_SPAN / Math.max(size.x, size.z, 1e-3));
      mesh.updateMatrixWorld(true);
      // Seat the floor at y=0 and centre the room on the stage (z=6).
      const box = new THREE.Box3().setFromObject(mesh);
      const centre = box.getCenter(new THREE.Vector3());
      mesh.position.set(-centre.x, -box.min.y, 6 - centre.z);
      mesh.traverse((o) => { if (o.isMesh) o.receiveShadow = true; });
      this.worldMesh = mesh;
      this.scene.add(mesh);
      (this.architectureShell || []).forEach((m) => { m.visible = false; });
      this.camera.far = 300;
      ...
      console.warn("World mesh unavailable; falling back to splat/gallery.", error);
      this.worldMesh = null;
      return false;
    }
  }
  ...
  async buildWorldEnvironment() {
    if (this.worldMeshUrl && (await this.buildWorldMesh())) return;
