...
export class Museum3D {
  ...
  placeArtworksInWorld(artworks) {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      // Floating frosted-glass display pane (user: the dark slab read cheap; wants a
      // translucent Apple-like look). No `transmission` — that forces a per-frame full-scene
      // pre-pass (the same cost we strip from companion materials); a clearcoated translucent
      // pane fakes the glass at zero extra passes. depthWrite off so splats behind stay visible.
      const panel = new THREE.Mesh(new THREE.BoxGeometry(1.8, 2.5, 0.08), new THREE.MeshPhysicalMaterial({ color: 0xeef2f6, transparent: true, opacity: 0.16, roughness: 0.1, metalness: 0, clearcoat: 1, clearcoatRoughness: 0.06, side: THREE.DoubleSide, depthWrite: false }));
      panel.position.set(0, 1.25, 0);
      panel.userData.artwork = artwork;
      group.add(panel);

      const frame = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 0.06), new THREE.MeshStandardMaterial({ color: 0xd9dee3, roughness: 0.26, metalness: 0.85 }));
