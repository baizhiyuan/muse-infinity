...
export class Museum3D {
  ...
  async loadWorldCollider() {
    ...
    try {
      ...
      if (this.worldScale !== 1) collider.scale.setScalar(this.worldScale); // stay aligned with the scaled mesh
  ...
  groundAt(x, z) {
    ...
    return hit ? hit.point.y : gy0;
  }

  // Hang artworks on the world's REAL interior walls: from the walk-path centre we raycast
  // outward to each side and mount the framed painting on the surface the ray hits (facing
  // back toward the path). In an enclosed world (e.g. the shimmering-spheres tunnel) this puts
  // the art on the actual walls. If there is no wall to hit (open / splat world) we fall back
  // to standing it at the walk-bound edge with a dark backboard. Artworks stay clickable
  // runtime planes with live metadata (spec) — never baked into the world.
  placeArtworksInWorld(artworks) {
    ...
    const pathC = alongX ? (this.worldProfile.spawn.z) : (this.worldProfile.spawn.x); // short-axis path centre
    const eyeY = (this.worldProfile.groundY || 0) + 1.5;
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      const onA = i % 2 === 0;
      const sideSign = onA ? -1 : 1;                     // alternate the two side walls
      const t = (Math.floor(i / 2) + 1) / (perSide + 1);
      const along = longMin + t * (longMax - longMin);

      const group = new THREE.Group();
      const frame = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 0.07), new THREE.MeshStandardMaterial({ color: 0xbda36d, roughness: 0.34, metalness: 0.6 }));
      ...
      // Fire from the path centre at eye height toward this side wall.
      const ox = alongX ? along : pathC;
      const oz = alongX ? pathC : along;
      const dir = alongX ? new THREE.Vector3(0, 0, sideSign) : new THREE.Vector3(sideSign, 0, 0);
      let placed = false;
      if (this.worldMesh) {
        ray.set(new THREE.Vector3(ox, eyeY, oz), dir);
        ray.far = 8;
        const hit = ray.intersectObject(this.worldMesh, true)[0];
        ray.far = Infinity;
        if (hit) {
          group.position.copy(hit.point).addScaledVector(dir, -0.12); // sit on the wall, slightly proud
          group.rotation.y = Math.atan2(-dir.x, -dir.z);              // face back toward the path
          placed = true;
        }
      }
      if (!placed) {
        // No real wall (open / splat world): stand it at the walk-bound edge + dark backboard.
        const wall = onA ? (alongX ? b.minZ : b.minX) : (alongX ? b.maxZ : b.maxX);
        const bx = alongX ? along : wall - sideSign * 0.2;
        const bz = alongX ? wall - sideSign * 0.2 : along;
        group.position.set(bx, this.groundAt(bx, bz) + 1.5, bz);
        group.rotation.y = Math.atan2(-dir.x, -dir.z);
        const backboard = new THREE.Mesh(new THREE.PlaneGeometry(1, 1), new THREE.MeshBasicMaterial({ color: 0x141414, side: THREE.DoubleSide, toneMapped: false }));
        backboard.scale.set(1.9, 1.5, 1);
    ...
    modelCompanions.forEach((companion, index) => {
      loader.load(
        ...
        gltf => {
          ...
          // Slots are camera-relative (back = metres behind camera; negative = ahead). Keep all
          // three CLOSE + ahead-flanking so they stay big and recognisable in-frame even with
          // the world scaled up 3× — far slots turned them into distant gold blobs.
          const slots = [{ back: -1.6, side: -1.1 }, { back: -1.6, side: 1.1 }, { back: -2.3, side: 0.0 }];
  ...
  updateCompanions() {
    ...
    for (const c of this.companionParty) {
      const a = c.avatar;
      const tx = cam.x - fwdX * c.back + rgtX * c.side;
      const tz = cam.z - fwdZ * c.back + rgtZ * c.side;
