...
export class Museum3D {
  ...
  placeArtworksInWorld(artworks) {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      painting.userData.artwork = artwork;
      group.add(painting);
      // Dark backboard so the painting reads as a hung panel, not floating in the void.
      ...
      const inset = 0.2;
      const px = alongX ? along : wall + (onA ? inset : -inset);
      const pz = alongX ? wall + (onA ? inset : -inset) : along;
      group.position.set(px, this.groundAt(px, pz) + 1.5, pz); // sit the frame on the real terrain height
      group.rotation.y = alongX ? (onA ? 0 : Math.PI) : (onA ? Math.PI / 2 : -Math.PI / 2);
      this.scene.add(group);
      this.paintings.push({ group, painting, artwork });
    }
  }
    ...
    modelCompanions.forEach((companion, index) => {
      loader.load(
        ...
        gltf => {
          ...
          // three AHEAD + flanking so they stay inside the ~58° FOV as the visitor walks — the
          // old behind-and-to-the-side layout let the third companion drift out of frame.
          const slots = [{ back: -2.0, side: -1.2 }, { back: -2.0, side: 1.2 }, { back: -3.2, side: 0.2 }];
