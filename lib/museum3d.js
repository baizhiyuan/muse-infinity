...
export class Museum3D {
  ...
  placeArtworksInWorld(artworks) {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      // feedback #13: flaky texture fetches left blank beige boards in-world while the same
      // image loaded fine in the popup. Retry once after a short delay; one retry recovers
      // most. If the retry also fails, leave the placeholder (existing behavior).
      loader.load(artwork.image, onTexture, undefined, () => {
        setTimeout(() => { if (!this.disposed) loader.load(artwork.image, onTexture); }, 1500);
      });
