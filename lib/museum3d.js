...
export class Museum3D {
  ...
  // Embed artworks as clickable planes IN the walkable world (spec: artworks stay runtime
  // planes, never baked, so metadata + click survive). Distributes them along the two long
  // walls of the walk bounds, at eye height, facing inward toward the path.
  placeArtworksInWorld(artworks) {
    ...
    const groundY = this.worldProfile.groundY || 0;
    const eyeH = groundY + 1.55;
    const alongX = (b.maxX - b.minX) >= (b.maxZ - b.minZ);
    const longMin = alongX ? b.minX : b.minZ;
    const longMax = alongX ? b.maxX : b.maxZ;
    const wallA = alongX ? b.minZ : b.minX;
    const wallB = alongX ? b.maxZ : b.maxX;
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      const onA = i % 2 === 0;
      const wall = onA ? wallA : wallB;
      const t = (Math.floor(i / 2) + 1) / (perSide + 1);
      const along = longMin + t * (longMax - longMin);

      const group = new THREE.Group();
      const frameMat = new THREE.MeshStandardMaterial({ color: 0xbda36d, roughness: 0.34, metalness: 0.6 });
      const frame = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 0.07), frameMat);
      frame.scale.set(1.3, 0.98, 1); // default landscape until the real image aspect is known
      group.add(frame);
      ...
      loader.load(artwork.image, (texture) => {
        ...
        // Size the plane to the image's REAL aspect ratio (no stretch); long side ~1.15 m.
      ...
      const inset = 0.2;
      if (alongX) {
        group.position.set(along, eyeH, wall + (onA ? inset : -inset));
        group.rotation.y = onA ? 0 : Math.PI;
      } else {
        group.position.set(wall + (onA ? inset : -inset), eyeH, along);
        group.rotation.y = onA ? Math.PI / 2 : -Math.PI / 2;
      }
      this.scene.add(group);
      this.paintings.push({ group, painting, artwork });
    }
    ...
    modelCompanions.forEach((companion, index) => {
      loader.load(
        ...
        gltf => {
          ...
          const slots = [{ back: 1.7, side: -0.95 }, { back: 2.1, side: 0.95 }, { back: 1.6, side: 0.0 }];
          if (this.worldProfile) {
            ...
            const gy = this.worldProfile.groundY || 0;
            avatar.position.set(s.x + slot.side, gy + footOffset, s.z + 1.2 + index * 0.3);
  ...
  pick(event) {
    ...
    // ONE combined call, recursive. Two calls with different flags would force us to hand-compare
    // distances and would get near/far ordering wrong; one call sorts everything for free.
    // Recursion is REQUIRED and is the subtle part: `loadCompanionModels` stamps
    // `userData.companion` on the gltf.scene ROOT, but the ray hits meshes several levels below it.
    // With `false` here every companion click silently misses. Recursing into a childless painting
    // plane is a no-op, so mixing the two lists costs nothing.
    const targets = [...this.paintings.map(item => item.painting), ...this.companionModels];
  ...
  updateCompanions() {
    ...
    for (const c of this.companionParty) {
      ...
      let gy = gy0;
      if (this.worldCollider) {
        this.raycaster.set(new THREE.Vector3(a.position.x, gy0 + EYE_HEIGHT + 1.0, a.position.z), DOWN);
        this.raycaster.far = 6;
        const hit = this.raycaster.intersectObject(this.worldCollider, true)[0];
        this.raycaster.far = Infinity;
        if (hit) gy = hit.point.y;
      }
      a.position.y = gy + c.footOffset;
      a.rotation.y = Math.atan2(fwdX, fwdZ); // face the way the visitor is walking
    }
  }
