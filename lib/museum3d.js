...
export class Museum3D {
  ...
  placeArtworksInWorld(artworks) {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      // SNAP TO THE REAL WALL when one exists (user: panes should live on the dense splat
      // surfaces so the gallery blends with the environment). Cast sideways from the path
      // centreline at picture height; a hit means a real wall — sit just proud of it. No hit
      // (open worlds) falls back to the freestanding lateral, clamped into walk bounds.
      const cx0 = alongX ? along : shortCenter;
      const cz0 = alongX ? shortCenter : along;
      const sideDir = new THREE.Vector3(alongX ? 0 : sideSign, 0, alongX ? sideSign : 0);
      const solidWall = this.worldMesh || this.worldCollider;
      let wallDist = null;
      if (solidWall) {
        this.raycaster.set(new THREE.Vector3(cx0, this.groundAt(cx0, cz0) + 1.5, cz0), sideDir);
        this.raycaster.far = lateral + 8;
        const wallHit = this.raycaster.intersectObject(solidWall, true)[0];
        this.raycaster.far = Infinity;
        if (wallHit && wallHit.distance > 1.2) wallDist = Math.max(1.2, wallHit.distance - 0.22);
      }
      ...
      // Museum vitrine, ALL UNLIT (Marble worlds carry no scene lighting; PBR renders black
      // there). Z-STACK strictly ordered so no front face ever passes the canvas:
      // glass slab [-0.05..0.05] -> opaque mat 0.06 -> walnut frame [0.06..0.10] ->
      // gold fillet [0.0725..0.1075] -> canvas 0.11 (frontmost).
      // The opaque mat WRITES DEPTH: gaussians behind the artwork are occluded instead of
      // smearing over it during motion (the original box gallery was crisp for this reason).
      // "Thickness" is sold by four luminous edge strips along the slab perimeter — the way
      // real glass catches light on its edges — instead of a flat cheap-white film.
      const panel = new THREE.Mesh(new THREE.BoxGeometry(1.8, 2.5, 0.1), new THREE.MeshBasicMaterial({ color: 0xdfeaf0, transparent: true, opacity: 0.15, depthWrite: false, toneMapped: false, side: THREE.DoubleSide }));
      panel.position.set(0, 1.25, 0);
      panel.userData.artwork = artwork;
      group.add(panel);
      const edgeMat = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.45, depthWrite: false, toneMapped: false });
      const edgeSpecs = [
        [1.84, 0.035, 0, 2.5], [1.84, 0.035, 0, 0],          // top / bottom strips (w, h, x, y)
        [0.035, 2.54, -0.9, 1.25], [0.035, 2.54, 0.9, 1.25], // left / right strips
      ];
      for (const [ew, eh, ex, ey] of edgeSpecs) {
        const edge = new THREE.Mesh(new THREE.BoxGeometry(ew, eh, 0.105), edgeMat);
        edge.position.set(ex, ey, 0);
        edge.userData.artwork = artwork;
        group.add(edge);
      }

      const matMat = new THREE.MeshBasicMaterial({ color: 0xf7f4ec, toneMapped: false });
      const backing = new THREE.Mesh(new THREE.PlaneGeometry(1, 1), matMat);
      backing.position.set(0, 1.5, 0.06);
      backing.scale.set(1.48, 1.16, 1);
      backing.userData.artwork = artwork;
      group.add(backing);

      // Walnut frame + thin gold fillet: the "wood frame" variant the user asked for, kept
      // restrained — warm wood against the cool glass, a gold line pulling toward the artwork.
      const frame = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 0.04), new THREE.MeshBasicMaterial({ color: 0x6e4c34, toneMapped: false }));
      frame.scale.set(1.32, 1.0, 1);
      frame.position.set(0, 1.5, 0.08);
      frame.userData.artwork = artwork;
      group.add(frame);
      const fillet = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 0.035), new THREE.MeshBasicMaterial({ color: 0xc9aa72, toneMapped: false }));
      fillet.scale.set(1.23, 0.91, 1);
      fillet.position.set(0, 1.5, 0.09);
      ...
      const onTexture = (texture) => {
        ...
        frame.scale.set(w + 0.14, h + 0.14, 1);
        fillet.scale.set(w + 0.05, h + 0.05, 1);
        backing.scale.set(w + 0.3, h + 0.3, 1);
      };
